...
function restoreAnalyticsState() {
  ...
  try {
    ...
    // Optional: expire old data (e.g. 30 min)
    ...
    Object.assign(anomalyStats, data.anomalyStats || {});
    ...
    // Restore anomaly log
    if (Array.isArray(data.anomalyLog)) {
      anomalyLog.length = 0;
      anomalyLog.push(...data.anomalyLog);
    }

    // Restore chart buffers
    if (data.chartBuffers) {
      Object.keys(data.chartBuffers).forEach(k => {
        chartBuffers[k] = data.chartBuffers[k];
      });
    }

  } catch (e) {
    console.warn('Failed to restore analytics state', e);
  }
}
...
function saveAnalyticsState() {
  try {
    // Trim buffers to avoid huge storage
    const trimmedBuffers = {};
    Object.keys(chartBuffers).forEach(k => {
      trimmedBuffers[k] = {};
      Object.keys(chartBuffers[k]).forEach(f => {
        trimmedBuffers[k][f] = chartBuffers[k][f].slice(-60); // keep last 60 points
      });
    });
    ...
    const data = {
      ...
      warnCount,
      okCount,
      anomalyLog: anomalyLog.slice(0, 20), // limit log size
      chartBuffers: trimmedBuffers,
      timestamp: Date.now()
    };

    localStorage.setItem('analyticsState', JSON.stringify(data));
  } catch (e) {
    console.warn('Failed to save analytics state', e);
  }
}
...
function refreshAnalyticsUI() {
  ...
  if (barChart) {
    barChart.data.datasets[0].data = ids.map(id => anomalyStats[id] || 0);
    barChart.update();
  }

  // ✅ FIX: update legend counts
  ids.forEach(id => {
    const el = document.getElementById('leg-' + id);
    if (el) el.textContent = anomalyStats[id] || 0;
  });

  // Restore anomaly list UI
  const colors = { alert:'var(--accent2)', warn:'var(--warn)', ok:'var(--ok)' };
  document.getElementById('an-list').innerHTML =
    anomalyLog.slice(0,10).map(e =>
      `<div style="display:flex;gap:10px;align-items:center;padding:6px 0;border-bottom:1px solid var(--border);">
        <span style="color:var(--muted);min-width:60px;">${e.t}</span>
        <span style="color:${colors[e.level]};min-width:50px;">${e.level.toUpperCase()}</span>
        <span style="color:var(--text);">${e.id}</span>
...
chartScript.onload = () => {
  restoreAnalyticsState();   // 👈 FIRST
  initChart();
  initBarChart();
  refreshAnalyticsUI();      // 👈 THEN RENDER
};
