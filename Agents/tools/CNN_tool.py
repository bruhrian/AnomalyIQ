# Already copied to CNN_train_val.py, 
# need to del away and replace
# with only calling of predict()
...
def build_aggregated_cnn(input_shape, num_aggregated_features):
    model = keras.Sequential([
        layers.Conv1D(filters=64, kernel_size=3, activation='relu', padding='causal', input_shape=input_shape),
        layers.BatchNormalization(),
        layers.MaxPooling1D(pool_size=2),
...
def train_aggregated_model(X_train, y_train, X_val, y_val, machine_type, save_dir):
    ...
    model = build_aggregated_cnn(input_shape, num_features)
    model.summary()
